      ...
      <main className="max-w-7xl mx-auto p-8 space-y-10">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold flex items-center gap-2">
            Edit Section: {getTranslatedField(section.key, activeLang, game.default_lang)}
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <section className="space-y-6 border-b pb-10">
              ...
              <form action={displaySettingsFormAction} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 md:col-span-2"><label className="block text-sm font-medium">Fields to Filter by</label><div className="flex flex-wrap gap-4 mt-2">{fields?.map(f => (<label key={f.id} className="flex items-center gap-2"><input type="checkbox" name="filter_field_ids" value={f.id} defaultChecked={displaySettings?.filter_field_ids?.includes(f.id)} />{getTranslatedField(f.key, activeLang, game.default_lang)}</label>))}</div></div>
        ...
        <EntityGridManager entities={entities} displaySettings={displaySettings} filterFields={filterFieldsData} gameSlug={game.slug} sectionId={section.id} sectionName={getTranslatedField(section.key, activeLang, game.default_lang)} isAdmin={true} gameDefaultLang={game.default_lang} activeLang={activeLang} />
      </main>
