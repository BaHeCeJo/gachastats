...
export default function NewOptionClient({ game, field, sectionId, currentLang: browserLang }: {
  game: GameData;
  field: FieldData;
  sectionId: string;
  currentLang: string;
  ...
  return (
    <GameLocalizationProvider gameDefaultLang={game.default_lang} gameSupportedLanguages={game.supported_languages}>
      <main className="max-w-xl p-8 space-y-6 mx-auto">
        <h1 className="text-2xl font-bold text-white">{t('newOption')} — {getTranslatedField(field.key, activeLang, game.default_lang)}</h1>
        ...
        <form action={formAction} className="space-y-4">
          <LocalizedTextInput id="value_key" label={t('optionValue')} value={localizedValueKey} onChange={setLocalizedValueKey} placeholder="e.g., Fire, Ice" />
          {field.has_color && (
            <div>
              <label htmlFor="color" className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2 ml-1">{t('color')}</label>
          ...
          <button type="submit" className="w-full bg-green-600 text-black font-bold px-4 py-3 rounded-xl hover:bg-green-500 transition-colors">{t('createOption')}</button>
        </form>
      </main>
